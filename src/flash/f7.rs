...
impl Flash {
    ...
    pub fn unlock(&mut self) {
        ...
        self.registers.keyr().write(|w| w.key().set(0x45670123));
        self.registers.keyr().write(|w| w.key().set(0xCDEF89AB));
    }
    ...
    fn clear_errors(&mut self) {
        self.registers.sr().write(|w| {
            w.erserr().clear_bit_by_one();
            w.pgperr().clear_bit_by_one();
            w.pgaerr().clear_bit_by_one();
            w.wrperr().clear_bit_by_one()
        });
    }
}
