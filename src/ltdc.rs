...
impl<T: 'static + SupportedWord> DisplayController<T> {
    ...
    pub fn new(
        ...
        rcc.dckcfgr.modify(|_, w| w.pllsaidivr().bits(pllsaidivr));
        ...
        ltdc.sscr.write(|w| {
            w.hsw().bits(config.h_sync - 1);
            w.vsh().bits(config.v_sync - 1)
        });
        ltdc.bpcr.write(|w| {
            w.ahbp().bits(config.h_sync + config.h_back_porch - 1);
            w.avbp().bits(config.v_sync + config.v_back_porch - 1)
        });
        ltdc.awcr.write(|w| {
            w.aaw()
                .bits(config.h_sync + config.h_back_porch + config.active_width - 1);
            w.aah()
                .bits(config.v_sync + config.v_back_porch + config.active_height - 1)
        });
        ltdc.twcr.write(|w| {
            w.totalw().bits(total_width);
            w.totalh().bits(total_height)
        });
    ...
    pub fn config_layer(
        ...
        _layer.whpcr.write(|w| {
            w.whstpos().bits(h_win_start + 1);
            w.whsppos().bits(h_win_start + width)
        });
        _layer.wvpcr.write(|w| {
            w.wvstpos().bits(v_win_start + 1);
            w.wvsppos().bits(v_win_start + height)
        });
