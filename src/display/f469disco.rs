...
//!
//! It is based on the `f469disco-lcd-test` example and the
//! `examples/f469disco-lcd-test.rs` helper.
// Based on STM32CubeF4 BSP LCD driver (STMicroelectronics, BSD-3-Clause)

#![allow(dead_code)]

use crate::{
    display::nt35510::Nt35510,
    dsi::{
...
pub fn init_dsi(dsi: DSI, rcc: &mut Rcc) -> DsiHost {
    ...
    let mut dsi_host =
        DsiHost::init(dsi_pll_config, DISPLAY_CONFIG, dsi_config, dsi, rcc).expect("DSI init");
...
pub fn init_panel(
    ...
    match controller {
        LcdController::Nt35510 => {
            ...
            if rgb565 {
                panel.init_rgb565(dsi_host, delay).expect("NT35510 init");
            } else {
                panel.init(dsi_host, delay).expect("NT35510 init");
            }
        }
        LcdController::Otm8009a => {
            #[cfg(feature = "defmt")]
            ...
            otm.init(dsi_host, otm_config, delay)
                .expect("OTM8009A init");
        }
    }
...
pub fn init_display_full(
    ...
    match controller {
        LcdController::Nt35510 => {
            ...
            panel.init(&mut dsi_host, delay).expect("NT35510 init");
        }
        LcdController::Otm8009a => {
            #[cfg(feature = "defmt")]
            defmt::info!("Initializing OTM8009A (B07 and earlier revisions)...");
            ...
            let mut otm = Otm8009A::new();
            otm.init(&mut dsi_host, otm_config, delay)
                .expect("OTM8009A init");
        }
    }
