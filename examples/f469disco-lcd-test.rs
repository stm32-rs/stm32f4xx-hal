...
fn main() -> ! {
    ...
    let mut dsi_host = match DsiHost::init(
        ...
        Err(e) => {
            ...
            let _ = e;
            #[cfg(feature = "defmt")]
            defmt::panic!("DSI host initialization failed: {:?}", e);
            #[cfg(not(feature = "defmt"))]
            panic!("DSI host initialization failed");
        }
    };
    ...
    match controller {
        LcdController::Nt35510 => {
            ...
            if let Err(_) = nt35510.init(&mut dsi_host, &mut delay) {
                #[cfg(feature = "defmt")]
                defmt::panic!("NT35510 init failed");
                #[cfg(not(feature = "defmt"))]
                panic!("NT35510 init failed");
            }
        }
        LcdController::Otm8009a => {
            ...
            if let Err(_e) = otm8009a.init(&mut dsi_host, otm8009a_config, &mut delay) {
                #[cfg(feature = "defmt")]
                defmt::panic!("OTM8009A init failed: {:?}", _e);
                #[cfg(not(feature = "defmt"))]
                panic!("OTM8009A init failed");
            }
        }
    }
    dsi_host.force_rx_low_power(false);
    #[cfg(feature = "defmt")]
    defmt::info!("force_rx_low_power cleared");
...
fn detect_lcd_controller(
    dsi_host: &mut DsiHost,
    delay: &mut (impl embedded_hal_02::blocking::delay::DelayUs<u32> + embedded_hal::delay::DelayNs),
) -> LcdController {
    #[cfg(feature = "defmt")]
    defmt::info!("Auto-detecting LCD controller...");

    const PROBE_RETRIES: u8 = 3;
    embedded_hal_02::blocking::delay::DelayUs::<u32>::delay_us(delay, 20_000u32); // Settle delay before probing
    let mut nt35510 = nt35510::Nt35510::new();
