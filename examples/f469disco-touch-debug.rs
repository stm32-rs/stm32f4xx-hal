...
// Simple display config for touch testing
...
fn main() -> ! {
    ...
    for pixel in buffer.iter_mut() {
        *pixel = 0xFFFF; // White in RGB565
    }

    // Draw a simple grid pattern to help with calibration
    // Draw red lines at known positions
    for y in 0..HEIGHT as usize {
        for x in 0..WIDTH as usize {
            let idx = y * WIDTH as usize + x;
            // Draw grid lines every 50 pixels
            if x % 100 == 0 || y % 100 == 0 {
                buffer[idx] = 0xF800; // Red in RGB565
            }
            // Mark corners
            if (x < 10 && y < 10) || (x > WIDTH as usize - 10 && y < 10) ||
               (x < 10 && y > HEIGHT as usize - 10) || (x > WIDTH as usize - 10 && y > HEIGHT as usize - 10) {
                buffer[idx] = 0x001F; // Blue corners
            }
        }
    }
    ...
    defmt::info!("=== TOUCH CALIBRATION DEBUG ===");
    defmt::info!("Grid pattern: red lines every 100px, blue corners");
    defmt::info!("Tap each corner and edge center, note coordinates");
    defmt::info!("Expected corners: (0,0), (479,0), (0,799), (479,799)");
    defmt::info!("==============================");
        ...
        match touch.detect_touch(&mut i2c) {
            ...
            Ok(n) if n > 0 => {
                match touch.get_touch(&mut i2c, 1) {
                    Ok(point) => {
                        defmt::info!("TOUCH: raw=({},{}) ntouch={}", point.x, point.y, n);
                    }
                    Err(e) => {
                        defmt::warn!("get_touch error: {:?}", e);
                    }
                }
            }
            _ => {}
